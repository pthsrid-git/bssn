  ...
  <div class="flex flex-col">
    ...
    <div v-for="(option, index) in options" :key="option.value" class="flex items-center mb-2">
      <input
        ...
        :class="radioComputedClasses"
        @change="updateValue(String(option.value))"
      />
      <label
        :for="`${name}-${index}`"
...
const radioComputedClasses = computed(() =>
  [
    'ml-2 h-4 w-4 text-primary-500 border-gray-300 focus:ring-primary-400',
...
const updateValue = (optionValue: string) => {
  handleChange(optionValue, props.submitCount > 0)
