  ...
  >
    <div class="space-y-6">
      ...
      <div>
        ...
        <div class="bg-white border border-gray-300 rounded-lg p-3 text-sm text-gray-900">
          {{ entry?.keterangan || entry?.deskripsi || 'Telah dibuat user interface dan telah disetujui oleh atasan langsung' }}
        </div>
      </div>
      ...
      <div class="flex justify-between items-center pt-4 w-full">
        ...
        <button
          ...
          class="px-6 py-2 bg-warning-500 text-black rounded-lg hover:bg-warning-600
                transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {{ approving ? 'Memproses...' : 'Simpan dan Setujui' }}
        </button>
      </div>
...
import { LogbookAtasanService } from '@/services/logbookAtasanService'
...
watch(() => props.entry, (newEntry) => {
  if (newEntry) {
    catatanAtasan.value = newEntry.catatan_atasan || ''
  }
}, { immediate: true })
...
const handleApprove = async () => {
  if (!props.entry?.id) {
    alert('ID logbook tidak ditemukan')
    return
  }
  ...
  saving.value = true
  try {
    await LogbookAtasanService.approveLog(props.entry.id, catatanAtasan.value)
...
const handleReject = async () => {
  ...
  if (!catatanAtasan.value.trim()) {
    alert('Harap masukkan alasan penolakan pada catatan atasan')
    return
  }

  // Konfirmasi
  if (!confirm('Apakah Anda yakin ingin menolak logbook ini?')) {
    return
  }

  saving.value = true
  try {
    await LogbookAtasanService.rejectLog(props.entry.id, catatanAtasan.value)
