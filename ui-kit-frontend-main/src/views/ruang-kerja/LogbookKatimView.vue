<template>
          ...
          <tr v-for="(member, index) in teamMembers" :key="member.id" class="hover:bg-gray-50">
            ...
            <TableData alignment="center">
              <button 
                ...
                @click="viewMemberLogbook(member)"
              >
                Lihat Logbook
              </button>
            </TableData>
  ...
  <!-- Card Logbook Anggota (Replace Card Daftar Anggota) -->
  <Card v-else>
    <div class="space-y-4">
      ...
      <div class="border-b border-gray-200 pb-4">
        ...
        >
          ...
          <span class="text-sm font-semibold">{{ selectedMember.nama }}</span>
        </button>
        
        <div class="flex items-center gap-1 text-sm text-gray-500">
          <span>{{ selectedMember.nip }}</span>
          <span>|</span>
          <span>{{ selectedMember.pangkat }}</span>
          <span>|</span>
          <span>{{ selectedMember.jabatan }}</span>
        </div>
      </div>
      ...
      <div v-else class="space-y-4">
        <CardDefault
          v-for="dateGroup in groupedMemberLogs"
          :key="dateGroup.date"
        ...
        <!-- Empty State -->
        <div v-if="groupedMemberLogs.length === 0" class="text-center py-12">
  ...
  <ModalDetailLogbookKatim ref="detailModal" :entry="selectedLog" @success="handleUpdateSuccess" />
</template>

<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'
import { LogbookKatimService, type TeamMember, type MemberLog } from '@/services/logbookKatimService'
...
import ModalDetailLogbookKatim from '@/components/modal/ModalDetailLogbookKatim.vue'
...
const selectedLog = ref<MemberLog | null>(null)
...
const groupedMemberLogs = computed(() => {
  const groups: any[] = []
...
const viewMemberLogbook = async (member: TeamMember) => {
...
const viewLogDetail = (log: MemberLog) => {
