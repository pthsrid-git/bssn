<!-- logbook-dws-frontend/src/components/Logbook/LogbookAtasanDetailModal.vue -->
<template>
  <div 
    v-if="show" 
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="flex items-start justify-between -mt-2">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Berikut adalah detail logbook pegawai</p>
          </div>
          <button 
            @click="handleClose"
        ...
        <div class="space-y-3">
          <div class="space-y-2">
            <div class="flex gap-4">
              <label class="text-sm font-medium text-gray-700 w-24">Tanggal</label>
              <div class="text-sm text-gray-900 font-bold">{{ formatDate(entry?.date) }}</div>
            </div>
            <div class="flex gap-4">
              <label class="text-sm font-medium text-gray-700 w-24">Waktu</label>
        ...
        <div>
          ...
          <input
            ...
            placeholder="Masukkan catatan untuk pegawai..."
          />
        </div>
        ...
        <div class="flex justify-between items-center pt-4 border-t border-gray-200">
          ...
          >
            ...
            ></span>
            {{ processing ? 'Menyimpan...' : 'Simpan dan Setujui' }}
          </button>
        </div>
      </div>
    </div>
...
const canApprove = computed(() => {
  const status = props.entry?.status?.toLowerCase() || ''
  return status === 'disetujui katim' || status === 'disubmit'
})

// Watch untuk sync data ketika modal dibuka
watch(() => props.entry, (newEntry) => {
  if (newEntry) {
    formData.value.catatan_atasan = newEntry.catatan_atasan || ''
  }
}, { immediate: true })
...
const handleApprove = async () => {
  ...
  try {
    const response = await logbookAtasanService.approveLog(
      props.entry.id,
      formData.value.catatan_atasan
    )
...
const handleReject = async () => {
  ...
  if (!formData.value.catatan_atasan || formData.value.catatan_atasan.trim() === '') {
    alert('Catatan atasan harus diisi untuk menolak logbook')
    return
  }
  ...
  try {
    const response = await logbookAtasanService.rejectLog(props.entry.id, {
      catatan_atasan: formData.value.catatan_atasan
    })
