    ...
    <div class="flex-1 overflow-hidden">
      <SectionDefault :sectionStatus="sectionStatus">
        <div class="w-full h-full flex flex-col gap-4 p-0.5 overflow-auto">
          ...
          <StateResponseError v-if="adminEperformaAdd.errorMessage" :message="adminEperformaAdd.errorMessage" />
        </div>
      </SectionDefault>
    </div>
...
const adminEperformaAdd = computed(() => adminEperformaStore.add)
...
const onSubmit = handleSubmit(async (values) => {
  ...
  await adminEperformaStore.callAdd(payload)
  if (adminEperformaAdd.value.status === 'success') {
    adminEperformaStore.clearAdd()
    onClose(true)
  }
})
