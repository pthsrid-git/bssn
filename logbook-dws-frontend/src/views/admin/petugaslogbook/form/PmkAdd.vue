    ...
    <div class="flex-1 overflow-hidden">
      <SectionDefault :sectionStatus="sectionStatus">
        <div class="w-full h-full flex flex-col gap-4 p-0.5 overflow-auto">
          ...
          <StateResponseError v-if="pmkAdd.errorMessage" :message="pmkAdd.errorMessage" />
        </div>
      </SectionDefault>
    </div>
...
const pmkAdd = computed(() => pmkStore.add)
...
const onSubmit = handleSubmit(async (values) => {
  ...
  await pmkStore.callAdd(payload)
  if (pmkAdd.value.status === 'success') {
    pmkStore.clearAdd()
    onClose(true)
  }
})
